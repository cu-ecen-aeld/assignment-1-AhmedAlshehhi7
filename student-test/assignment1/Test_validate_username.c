...
{
    const char* expected_username = my_username();  // Get hardcoded username
    char* actual_username = malloc_username_from_conf_file();  // Read from conf/username.txt

    TEST_ASSERT_EQUAL_STRING_MESSAGE(expected_username, actual_username, "Username validation failed!"); 

    free(actual_username);  // Free allocated memory
}
